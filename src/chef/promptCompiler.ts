...
- Never save a recipe merely because you recommended or wrote it. For a concrete recipe worth offering to the cookbook, you may call recipe_save with propose=true so the user gets a recipe preview and can add it themselves. Use propose=false only when the user explicitly asks to save that recipe. If a meal contains multiple distinct recipes, call recipe_save separately for each recipe so each can be reviewed or saved independently. If application allergy validation rejects a recipe, revise it before presenting it as saved.
